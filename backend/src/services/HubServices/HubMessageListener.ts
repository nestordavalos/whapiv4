...
const HubMessageListener = async (
  ...
  whatsapp: Whatsapp,
  _medias: Express.Multer.File[]
) => {
  if ((message as any).direction === "IN") {
    (message as any).fromMe = false;
  }

  const ignoreEvent = (message as any).direction === "OUT";
  if (ignoreEvent) return;

  const isMsgJoinInvite = (message as any).contents === null;
  if (isMsgJoinInvite) return;
  ...
  if (isMessageFromMe) {
    ...
    if (isMessageSent) {
      UpdateMessageAck((message as HubConfirmationSentMessage).messageId);
    } else {
      console.log(
        "HubMessageListener: message not sent",
        (message as HubConfirmationSentMessage).messageStatus.code,
        (message as HubConfirmationSentMessage).messageStatus.description
      );
    }
